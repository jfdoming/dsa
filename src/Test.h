...
template <typename T, typename U>
void test_assert_not_equal(T t, U u, const char *file, int line, const char *arg) {
    if (t == u) {
        std::ostringstream oss;
        oss << "Expected " << t << " but got " << u << " instead." << std::endl;
        _log_prefix(oss, file, line);
        oss << "TEST_ASSERT_NOT_EQUAL(" << arg << ");" << std::endl;
        throw AssertionException{oss.str()};
    }
}
...
void test_assert_equal_q(T t, U u, const char *file, int line, const char *tString, const char *uString) {
    if (!(t == u)) {
        std::ostringstream oss;
        oss << "Expected " << tString << " to equal " << uString << "." << std::endl;
        _log_prefix(oss, file, line);
...
template <typename T, typename U>
void test_assert_not_equal_q(T t, U u, const char *file, int line, const char *tString, const char *uString) {
    if (t == u) {
        std::ostringstream oss;
        oss << "Expected " << tString << " to not equal " << uString << "." << std::endl;
        _log_prefix(oss, file, line);
        oss << "TEST_ASSERT_NOT_EQUAL_Q(" << tString << ", " << uString << ");" << std::endl;
        throw AssertionException{oss.str()};
    }
}
...
#define TEST_ASSERT_NOT_EQUAL(lhs, rhs) test_assert_not_equal(lhs, rhs, __FILE__, __LINE__, #lhs" != "#rhs);
#define TEST_ASSERT_NOT_EQUAL_Q(lhs, rhs) test_assert_not_equal_q(lhs, rhs, __FILE__, __LINE__, #lhs, #rhs);
